...
describe("accounts", () => {
  ...
  it("User can add an account, edit an existing amount, and then delete the account", () => {
    // Review account balance
    let oldBalance;
    cy.get(".budget-total-amount").then(
      (balance) => (oldBalance = balance.text())
    ...
    // Clear the money amount and add new amount
    cy.get("#et-working-balance").focus().clear();
    cy.get("#et-working-balance").type(200);
    ...
    // Find accounted edited New account with name and starting balance should be visible on screen
    cy.findByText(name);
    let newAccountAmount;
    cy.get(`#${name}amount`).then(
      (amount) => (newAccountAmount = amount.text())
    );
